...
function SimpleOrbit(; period, duration, t0=zero(period), b=0)
    ...
    return SimpleOrbit(period, t0, b, duration, speed, half_period, ref_time)
end

period(orbit::SimpleOrbit) = orbit.period
duration(orbit::SimpleOrbit) = orbit.duration

function relative_time(orbit::SimpleOrbit, t)
    return mod(t - orbit.ref_time, period(orbit)) - orbit.half_period
end
...
function flip(orbit::SimpleOrbit, ror)
    ...
    ref_time = orbit.t0
    return SimpleOrbit(
        period(orbit), t0, b, duration(orbit), speed, orbit.half_period, ref_time
    )
end
...
function Base.show(io::IO, orbit::SimpleOrbit)
    ...
    b = orbit.b
    t0 = orbit.t0
    return print(io, "SimpleOrbit(P=$P, T=$T, t0=$t0, b=$b)")
end
